...
import {
    ...
    X, CheckCircle, XCircle, Calendar, Eye, Edit2
} from 'lucide-react';
        ...
        <div className="flex h-screen w-full overflow-hidden bg-[#f6f7f8] dark:bg-[#0f172a] text-slate-900 dark:text-slate-50 font-sans transition-colors duration-200">
            ...
            <main className="flex-1 flex flex-col overflow-hidden relative">
                ...
                <div className="flex-1 overflow-auto p-4 md:p-8 space-y-8 scroll-smooth transition-colors duration-200">
                    ...
                    {/* Analytics Cards - Completion, Average Score, Top Performer */}
                    {selectedJob && candidates.length > 0 && (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {/* Completion Rate */}
                            <div className="bg-gradient-to-br from-blue-600 to-blue-700 dark:from-blue-900 dark:to-blue-950 p-8 rounded-[2rem] border border-blue-500 dark:border-blue-800 shadow-lg">
                                <p className="text-[10px] font-black text-blue-200 uppercase tracking-[0.2em] mb-2">Completion Rate</p>
                                <p className="text-5xl font-black text-white mb-2">
                                    {candidates.length > 0 ? '100%' : '0%'}
                                </p>
                                <div className="w-full h-2 bg-blue-800 dark:bg-blue-950 rounded-full overflow-hidden mb-2">
                                    <div className="h-full bg-blue-300" style={{ width: candidates.length > 0 ? '100%' : '0%' }}></div>
                                </div>
                                <p className="text-xs text-blue-100 font-bold">All {candidates.length} candidates completed</p>
                            </div>

                            {/* Average Score */}
                            <div className="bg-gradient-to-br from-emerald-600 to-emerald-700 dark:from-emerald-900 dark:to-emerald-950 p-8 rounded-[2rem] border border-emerald-500 dark:border-emerald-800 shadow-lg">
                                <p className="text-[10px] font-black text-emerald-200 uppercase tracking-[0.2em] mb-2">Average Score</p>
                                <p className="text-5xl font-black text-white mb-2">
                                    {candidates.length > 0
                                        ? `${(candidates.reduce((sum, c) => sum + (c.percentage || 0), 0) / candidates.length).toFixed(1)}%`
                                        : '0%'
                                    }
                                </p>
                                <div className="w-full h-2 bg-emerald-800 dark:bg-emerald-950 rounded-full overflow-hidden mb-2">
                                    <div
                                        className="h-full bg-emerald-300"
                                        style={{
                                            width: candidates.length > 0
                                                ? `${(candidates.reduce((sum, c) => sum + (c.percentage || 0), 0) / candidates.length)}%`
                                                : '0%'
                                        }}
                                    ></div>
                                </div>
                                <p className="text-xs text-emerald-100 font-bold">
                                    Pass Rate: {candidates.length > 0
                                        ? `${((candidates.filter(c => c.percentage >= (selectedJob.minScore || 60)).length / candidates.length) * 100).toFixed(0)}%`
                                        : '0%'
                                    }
                                </p>
                            </div>
                            ...
                            {/* Top Performer */}
                            <div className="bg-gradient-to-br from-purple-600 to-purple-700 dark:from-purple-900 dark:to-purple-950 p-8 rounded-[2rem] border border-purple-500 dark:border-purple-800 shadow-lg">
                                <p className="text-[10px] font-black text-purple-200 uppercase tracking-[0.2em] mb-2">Top Performer</p>
                                <p className="text-5xl font-black text-white mb-2">
                                    {candidates.length > 0
                                        ? `${Math.max(...candidates.map(c => c.percentage || 0)).toFixed(0)}%`
                                        : '0%'
                                    }
                                </p>
                                <div className="w-full h-2 bg-purple-800 dark:bg-purple-950 rounded-full overflow-hidden mb-2">
                                    <div
                                        className="h-full bg-purple-300"
                                        style={{
                                            width: candidates.length > 0
                                                ? `${Math.max(...candidates.map(c => c.percentage || 0))}%`
                                                : '0%'
                                        }}
                                    ></div>
                                </div>
                                <p className="text-xs text-purple-100 font-bold">
                                    {(() => {
                                        ...
                                        const topCandidate = candidates.reduce((max, c) =>
                                            ...
                                            , candidates[0]);
                                        return topCandidate.user?.name || topCandidate.mockUser?.name || 'Unknown';
                                    })()}
                                </p>
                            </div>
                        </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            {jobs.map(job => (
                                <div
                                    key={job.id}
                                    className={`p-6 rounded-[2rem] border-2 cursor-pointer transition-all relative group ${selectedJob?.id === job.id
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        className="absolute top-4 right-4 p-2 bg-slate-100 dark:bg-slate-800 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity hover:bg-slate-200 dark:hover:bg-slate-700"
                                        title="Edit Assessment"
                                    >
                                        <Edit2 className="w-4 h-4 text-slate-600 dark:text-slate-400" />
                                    </button>

                                    <div onClick={() => handleViewCandidates(job)}>
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            <div className="space-y-4">
                                                {selectedJob.questions.map((q, idx) => {
                                                    // Get the actual user answer from the answers object
                                                    const userAnswer = selectedCandidate.answers?.[idx];
                                                    ...
                                                    // Check AI evaluation details for correctness
                                            ...
                                            return (
                                                <div key={i} className="bg-slate-50 dark:bg-slate-800/30 rounded-2xl p-6 border border-slate-100 dark:border-slate-800">
                                                    <div className="flex justify-between items-start mb-3">
                                                        ...
                                                        <span className={`px-2 py-1 rounded text-[9px] font-black uppercase tracking-widest ${status === 'success' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' :
